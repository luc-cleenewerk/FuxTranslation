...
void* create_new_problem(int size, int lower_bound_domain, int upper_bound_domain, int species, int* cantusFirmus, int pcost, int mtricost, 
    ...
    writeToLogFile(to_string(sizeof(cantusFirmus)).c_str());
    write_to_log_file("testing write_to_log_file");
    vector<int> cf(int_pointer_to_vector(cantusFirmus, size));

    writeToLogFile(to_string(cf.size()).c_str());
    writeToLogFile(int_vector_to_string(cf).c_str());

    writeToLogFile("cf vector created");
    vector<int> sp(int_pointer_to_vector(splist, sizeof(splist)));
    writeToLogFile("sp vector created");
    writeToLogFile(to_string(sizeof(splist)).c_str());
    writeToLogFile(to_string(sp.size()).c_str());
    writeToLogFile(int_vector_to_string(sp).c_str());
