...
void link_harmonic_arrays_1st_species(const Home &home, int size, vector<Part> parts,vector<Stratum> lowest, 
    ...
    for(int j = 0; j < parts.size(); j++){
        for(int i = 0; i < size; i++){
            rel(home, parts[j].hIntervalsCpCf[i] == abs(parts[j].notes[i] - lowest[0].notes[i]));
        }
    }

    for(int p = 0; p < parts.size()-1; p++){
        for(int i = 0; i < size; i++){
            rel(home, expr(home, lowest[0].notes[i]-upper[p].notes[i]), IRT_EQ, upper[p].hIntervalsBrut[i]);
            abs(home, upper[p].hIntervalsBrut[i], upper[p].hIntervalsAbs[i]);
        }
    }
}
...
void link_cfb_arrays_1st_species(const Home &home, int size, vector<Part> parts){
    ...
    for(int k = 1; k < parts.size(); k++){
        for(int i = 0; i < size; i++){
            rel(home, parts[k].notes[i], IRT_GQ, parts[0].notes[i], Reify(parts[k].isCFB[i], RM_EQV));
        }
    }
}
...
void link_melodic_arrays_1st_species(const Home &home, int size, vector<Part> parts){
    ...
    for(int k = 0; k < parts.size(); k++){
        for(int i = 0; i < size-1; i++){
            rel(home, expr(home, parts[k].notes[i]-parts[k].notes[i+1]), IRT_EQ, parts[k].m_intervals_brut[i]);
            abs(home, parts[k].m_intervals_brut[i], parts[k].m_intervals[i]);
        }
    }
}
     ...
     vector<Stratum> lowest){
        for(int p = 1; p < parts.size(); p++){
            for(int i = 0; i < size-1; i++){
                ...
                BoolVar both_up = expr(home, (parts[p].m_intervals_brut[i]>0)&&(parts[0].m_intervals_brut[i]>0));
                BoolVar both_stay = expr(home, (parts[p].m_intervals_brut[i]==0)&&(parts[0].m_intervals_brut[i]==0));
                BoolVar both_down = expr(home, (parts[p].m_intervals_brut[i]<0)&&(parts[0].m_intervals_brut[i]<0));
                //oblique motions help creation
                BoolVar cf_stays_1 = expr(home, (parts[p].m_intervals_brut[i]>0)&&(parts[0].m_intervals_brut[i]==0));
                BoolVar cf_stays_2 = expr(home, (parts[p].m_intervals_brut[i]<0)&&(parts[0].m_intervals_brut[i]==0));
                BoolVar cp_stays_1 = expr(home, (parts[p].m_intervals_brut[i]==0)&&(parts[0].m_intervals_brut[i]>0));
                BoolVar cp_stays_2 = expr(home, (parts[p].m_intervals_brut[i]==0)&&(parts[0].m_intervals_brut[i]<0));
                //contrary motions help creation
                BoolVar cpd_cfu = expr(home, (parts[p].m_intervals_brut[i]<0)&&(parts[0].m_intervals_brut[i]>0));
                BoolVar cpu_cfd = expr(home, (parts[p].m_intervals_brut[i]>0)&&(parts[0].m_intervals_brut[i]<0));
                //direct constraints
...
void perfect_consonance_constraints(const Home &home, int size, vector<Part> parts, int n_species){
    ...
    if(n_species==1){
        for(int p = 0; p < parts.size(); p++){
            dom(home, parts[p].hIntervalsCpCf[0], perfect_consonance);
            dom(home, parts[p].hIntervalsCpCf[size-1], perfect_consonance);
        }
    }
}
...
void key_tone_tuned_to_cantusfirmus(const Home &home, int size, vector<Part> parts){
    //works
    for(int p = 1; p < parts.size(); p++){
        rel(home, (parts[p].isCFB[0] == 0) >> (parts[p].hIntervalsCpCf[0]==0));
        rel(home, (parts[p].isCFB[size-1] == 0) >> (parts[p].hIntervalsCpCf[size-1]==0));
    }
}
        ...
        for(int p2 = 0; p2 < parts.size(); p2++){
            if(p2!=p1){
                for(int i = 1; i < size-1; i++){
                    rel(home, parts[p1].notes[i], IRT_NQ, parts[p2].notes[i]);
                }
            }
        }
    }
...
void penultimate_note_must_be_major_sixth_or_minor_third(const Home &home, int size, vector<Part> parts){
    ...
    if(parts.size()==3){
        ...
        rel(home, parts[1].hIntervalsCpCf[p], IRT_EQ, 3, Reify(parts[0].is_not_lowest[p], RM_IMP));
        rel(home, parts[1].hIntervalsCpCf[p], IRT_EQ, 9, Reify(parts[0].is_not_lowest[p], RM_IMP));
    }
}
...
void no_tritonic_intervals(const Home &home, int size, int costtri, vector<Part> parts){
    ...
    for(int p = 1; p < parts.size(); p++){
        for(int j = 0; j < size-1; j++){
            rel(home, (parts[p].m_intervals[j]==6) >> (parts[p].M_deg_cost[j]==costtri));
            rel(home, (parts[p].m_intervals[j]!=6) >> (parts[p].M_deg_cost[j]==0));
        }
    }
}
...
void melodic_intervals_not_exceed_minor_sixth(const Home &home, int size, vector<Part> parts){
    ...
    for(int p = 1; p < parts.size(); p++){
        for(int j = 0; j < size-1; j++){
            rel(home, parts[p].m_intervals[j], IRT_LQ, 8);
        }
    }
}
...
void last_chord_no_minor_third(const Home &home, int size, vector<Part> parts){
    for(int p = 0; p < parts.size(); p++){
        if(parts[p].is_not_lowest[size-2].assigned()){
            if(parts[p].is_not_lowest[size-2].val()==1){
                rel(home, parts[p].hIntervalsCpCf[size-2], IRT_NQ, 3);
            }
        }
    }
}
