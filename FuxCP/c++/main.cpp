...
void run_bab_solver(Problem* p);
void run_dfs_solver(Problem* p);
void run_dfs_bound_solver(Problem* p);
...
int main(int argc, char* argv[]) {
    ...
    vector<int> speciesList = {1, 1, 1};  // @damien to test 2sp, put {1,2,1}
...
void run_dfs_bound_solver(Problem* p){
    ...
    while(Problem * pb = get_next_solution_space(e)){
        nb_sol++;
        if (nb_sol % 50000 == 0) {cout << "solver is at solution number " << nb_sol << endl;}
        current_cost = pb->cost().val();
